...
class WellsChart extends Component {
    ...
    geraGrafico() {
        ...
        return (
            <BarChart width={820} height={380} data={data} margin={{top: 20, right: 5, left: 20, bottom: 5 }}>
                ...
                <Bar dataKey="valores"  label={{ position: 'top' }}>
                    <LabelList dataKey="score" position="top"/>
                    {
                        data.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={this.geraBarra(data[index].score)} Label datakey={data[index].valores} position="top" strokeWidth={index === 2 ? 4 : 1}  />
                        ))
                    }
                </Bar>
            </BarChart>
